...
const CenterConfirm: React.FC<CenterConfirmProps> = ({ request, onResolve }) => {
    return (
        <div className="fixed inset-0 z-[201] flex items-center justify-center bg-black/70 backdrop-blur-sm px-6">
            ...
            >
                <p className="text-[10px] font-bold uppercase tracking-[0.4em]" style={{ color: 'var(--app-text-faint)' }}>{request.title ?? 'Confirm'}</p>
                <p className="mt-4 font-mono text-sm" style={{ color: 'var(--app-text)' }}>{request.message}</p>
                <div className="mt-6 flex gap-4">
                    <button
                        onClick={() => onResolve(false)}
                        className="w-full rounded-2xl px-6 py-3 text-[9px] font-bold uppercase tracking-[0.3em] transition-all border focus:outline-none focus-visible:ring-2 focus-visible:ring-white/50"
                    ...
                    <button
                        ...
                        className="w-full rounded-2xl px-6 py-3 text-[9px] font-bold uppercase tracking-[0.3em] transition-all hover:scale-105 shadow-xl focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500"
